...
export default class GoogleDriveStore {
    ...
    setKey = () => {
        ...
        this.client_id = process.env.GD_CLIENT_ID || 'placeholder_client_id';
        this.app_id = process.env.GD_APP_ID || 'placeholder_app_id';
        this.api_key = process.env.GD_API_KEY || 'placeholder_api_key';
        
        // Check if real credentials are provided
        const hasRealCredentials = this.client_id !== 'placeholder_client_id' && 
                                 this.app_id !== 'placeholder_app_id' && 
                                 this.api_key !== 'placeholder_api_key';
        
        if (!hasRealCredentials && process.env.NODE_ENV === 'development') {
            // Only show detailed info in development
            console.info('💡 Google Drive integration disabled - using placeholder credentials');
            console.info('To enable: Set GD_CLIENT_ID, GD_APP_ID, GD_API_KEY in .env file');
        }
    ...
    initialiseClient = () => {
        // Check if we have real credentials before initializing
        if (this.client_id === 'placeholder_client_id') {
            // Silently skip initialization with placeholder credentials
            return;
        }
        ...
        try {
            this.client = google.accounts.oauth2.initTokenClient({
                ...
                callback: (response: { expires_in: number; access_token: string; error?: TErrorWithStatus }) => {
                    ...
                    } else if (response?.error && process.env.NODE_ENV === 'development') {
                        console.error('❌ Google Drive authentication error:', response.error);
                    }
                },
            });
        } catch (error) {
    ...
    async signIn() {
        if (!this.client) {
            console.warn('⚠️ Google Drive client not initialized - check credentials');
            botNotification(
                'Google Drive not configured. Please check environment variables.',
                undefined,
                { closeButton: true }
            );
            return;
        ...
        if (!this.is_authorised) {
            try {
                ...
                botNotification(
                    'Google Drive sign-in failed. Please check your configuration.',
                    undefined,
                    { closeButton: true }
                );
            }
